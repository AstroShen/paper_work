...
print(channel_width)
for lane_num in range(4, 5):
    for I in range((N+1)*K//2, 71):
        if (K*N) % lane_num == 0 and I % lane_num == 0:
            for lim_size in range(max(channel_width//I, 4), 3*channel_width//I+1):
                if lim_size % 2 == 1:
                    continue
...
with open('config.txt', 'wa') as fp:
    ...
    fp.write('\n'.join(arch_list_add))
    fp.write('\nscript_params=-starting_stage vpr --route_channel_width %d' % channel_width)
