...
def add_leim_node(G):
    ble_cnt = 0
    lane_cnt = 0
    lane_dist = [int(K*N/Lane_num) if i < Lane_num-int(K*N % Lane_num)
                 else int(K*N/Lane_num)+1 for i in range(0, Lane_num)]
    ...
    for i in range(0, K*N):
        ...
        G.add_node(name, node_type='leim', lane=lane_cnt,
                   ble=ble_cnt, ble_i_cnt=lut_input_cnt)


def add_lim_node(G):
    lane_dist = [int(I/Lane_num) if i < Lane_num-int(I % Lane_num)
                 else int(I/Lane_num)+1 for i in range(0, Lane_num)]
    lane_deli = [0 if i == 0 else sum(lane_dist[0:i])
                 for i in range(0, len(lane_dist))]
    for i in range(I):
        lane_cnt = -1
...
def add_sb2sb_edge(G):
    ...
    def get_wires_by_beg(wires, begin): return [wires[i] for i in range(
        len(wires)) if wires[i][-1]['begin'] == begin]
        ...
        for turn in wilton_offset_dict:
            ...
            for sb_rule in sb_rules:
                ...
                src_wires_len_of_range = tuple(
                    set([wire['length'] for wire in H_wires + V_wires if wire['range'] == src_range]))
                sink_wires_len_of_range = tuple(set(
                    [wire['length'] for wire in H_wires + V_wires if wire['range'] == sink_range]))
    ...
    if Interlane_con_valid:
        ...
        for ble in range(N):
            ...
            for driving_ble in driving_bles:
                wires_of_driving_ble = wires_of_driving_ble + \
                    get_wires_by_beg(wires_by_ble[driving_ble], 1)
            for wire, attrs in wires_of_ble:
                node_type = attrs['node_type']
                length = attrs['length']
                direction = attrs['direction']
                wire_cnt = attrs['wire_cnt']
                driving_wires = list(filter(lambda x: x[1]['node_type'] == node_type and x[1]['length'] ==
                                     length and x[1]['direction'] == direction and x[1]['wire_cnt'] == wire_cnt, wires_of_driving_ble))
                for driving_wire in driving_wires:
                    G.add_edge(driving_wire[0], wire,
                               mux_type='L%d_medium' % length)


def add_out2sb_edge(G):
    out_rules = [pat for pat in General_route_rule if pat[0] == 'O']
    output_nodes = [node for node in G.nodes(
        data=True) if node[1]['node_type'] == 'output']

    def get_output_by_ble(ble): return [
        node for node in output_nodes if node[1]['ble'] == ble]
    wire_beg_nodes = [node for node in G.nodes(data=True) if node[1]['node_type'] in (
        'h_track', 'v_track') and node[1]['begin'] == 1]
...
def generate_arch_file(G):
    ...
    for wire in H_wires:
        ...
        cb_pattern = '1' + max(length-2, 0)*' 0' + \
            (' 1' if length != 1 else '')
        freq = float(wire['num_per_lut']*wire['length'])
        segment_inst = segment_template % (
            freq, length, name, name.lower(), sb_pattern, cb_pattern)
    ...
    mux_detail_template = '\t<from from_detail="%s" name="%s" switchpoint="0" type="%s"/>\n'
        ...
        if node_type == 'lim':
            ...
            for src_node in G.predecessors(node):
                ...
                if src_attrs['node_type'] in ('h_track', 'v_track'):
                    ...
                    mux_inst += mux_detail_template % ('%s:%d' % (
                        'o' if src_attrs['ble_i_cnt'] == 0 else 'q', src_attrs['ble']), 'plb', 'pb')
                else:
            ...
            for src_node in G.predecessors(node):
                ...
                if src_attrs['node_type'] == 'lim':
                    from_mux += '%s-%d ' % (src_attrs['node_type'],
                                            src_attrs['I'])
                elif src_attrs['node_type'] == 'output':
                    # TODO: i assume the output can have at most 2 outputs (o, q) per ble
                    mux_inst += mux_detail_template % ('%s:%d' % (
            ...
            mux_name = '%s-L%d-%d' % (attrs['direction'], attrs['length'],
            ...
            for src_node in G.predecessors(node):
                ...
                if src_attrs['node_type'] in ('h_track', 'v_track'):
                    if src_attrs['begin'] == 1:
                        from_first_stage_mux.append('%s_ble_%d_L%d_%d' % (
                            src_attrs['direction'], src_attrs['ble'], src_attrs['length'], src_attrs['ble']*src_attrs['num_per_lut']+src_attrs['wire_cnt']))
                    else:
                        mux_inst += mux_detail_template % ('%s%d' % (reverse_dir[src_attrs['direction']], src_attrs['ble']
                                                                     * src_attrs['num_per_lut']+src_attrs['wire_cnt']), 'L%d' % src_attrs['length'], 'seg')
                elif src_attrs['node_type'] == 'output':
            ...
            for first_stage_mux in from_first_stage_mux:
                if first_stage_mux not in first_stage_driver_mux_inst_list:
                    from_detail = first_stage_mux.split(
                        '_')[0] + first_stage_mux.split('_')[-1]
    ...
    for wire in H_wires:
        seg_group_inst = seg_group_template % (
            'L%d' % wire['length'], N*wire['num_per_lut'])
    ...
    arch_file_template = open(os.path.join(os.path.dirname(target_dir), template_file_name), 'rt')
    ...
    with open(os.path.join(target_dir, arch_file_inst), 'wt') as fp:
        for line in arch_file_template:
            ...
            line = re.sub(first_stage_driver_mux_pat,
                          first_stage_driver_mux_inst_str, line)
...
if __name__ == "__main__":
    ...
    print('channel_width: ', 2*N *
          sum(wire['length']*wire['num_per_lut'] for wire in H_wires))
    # print(H_wires)
    # print(V_wires)
